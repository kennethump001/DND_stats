...
function rollstats(stat){
    ...
    var statsum = 0;
    for(i =0; i<4; i++){
        var d6 = Math.floor(Math.random() * (6 - 1) +1);
        stats.push(d6);
    }
    statsmin = Math.min(...stats);
    statsum = stats.reduce((total, current) => total + current, 0);
    document.getElementById(stat).innerHTML = Math.floor(statsum - statsmin).toString();
